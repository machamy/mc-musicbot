...
> ⚠️ **반드시 `git clone`으로 받으세요.** GitHub "Download ZIP"으로 받으면 git 이력이 없어 나중에 업데이트(`update-and-run` = `git pull`)가 동작하지 않습니다.
...
>
> **YouTube 쿠키는 필수가 아닙니다.** yt-dlp는 기본적으로 **쿠키 없이(공개 접근)** 동작하고, 봇은 *브라우저 쿠키 → 쿠키 파일 → 공개 접근* 순으로 시도해 대부분 공개 접근만으로 재생됩니다. 연령제한·지역제한·회원전용 영상이거나 YouTube가 봇으로 의심해 막을 때만 쿠키가 도움이 되며, 그땐 **웹 관리자 → 재생 설정**에서 "선호 브라우저 프로필"(예: `chrome` / `edge`) 또는 "쿠키 파일 경로"(Netscape 형식)를 지정하면 됩니다.
...
**재생 / 소스**
- YouTube · YouTube Music · SoundCloud (`yt-dlp` + `ffmpeg`)
- URL 직접 재생, 플레이리스트·앨범 URL은 펼쳐서 대기열에 추가
- 디스코드 안 검색(`/검색`=YouTube, `/사클검색`=SoundCloud) 후 셀렉트 메뉴로 골라 재생
- 특정 시간으로 이동(`/이동시간`), 다시재생, 이전곡
- 곡 추가/검색 응답에 ✖ 취소 버튼, 제목은 원본 URL 하이퍼링크

**대기열**
- 반복: 없음 / 한곡 / 전체(큐 전체 루프)
- 셔플, 순서 이동(`/이동`), 제거(`/제거`), 비우기(`/큐비우기`), 지정 순번으로 점프(`/지정스킵`)
- 현재곡 카드(진행바·썸네일·다음곡)와 재생 컨트롤 버튼
...
**캐시**
- 받은 곡 로컬 LRU 캐시 → 같은 곡은 다시 안 받고 즉시 재생, 용량 한도 자동 정리

**운영 / 관리**
- 차단 목록(제목·URL, 길드/전역), 저장 플레이리스트(전역/길드)
- 빈 음성 채널 자동 퇴장 정책(없음/정지/퇴장 + 지연)
- 곡 시작 알림 카드
- **yt-dlp 자동 다운로드 + 하루 1회 자동 업데이트**
- 웹 관리자 패널(아래), 최초 비밀번호 설정(localhost) + 변경
- 한국어 + 영어 명령, 길드 즉시 등록(`registerGuildId`) 옵션

---

## 자동추천(라디오) 작동 방식

`/자동추천 켜기`(또는 웹의 "자동추천 기본값")를 켜면 **대기열이 비어도 비슷한 곡을 라디오처럼 계속 이어 줍니다.** 동작은 다음과 같습니다.

1. **시드(seed) 선정** — 방금 재생한 곡(없으면 최근 곡)을 기준으로 삼습니다.
2. **후보 수집** (순서대로 시도):
   1. **공급자 라디오 믹스** — YouTube `RD`(라디오) · YouTube Music `RDAMVM` · SoundCloud `/recommended` 스테이션을 `yt-dlp`로 펼쳐 후보를 모읍니다.
   2. **제목+아티스트 검색** — 라디오가 비면 시드의 제목·아티스트로 검색합니다.
   3. **최후 보루** — 그래도 없으면 모은 후보 중 시드가 아닌 곡 아무거나.
3. **거르기**:
   - **최근 25곡 + 현재/대기열 곡 제외** — 같은 곡이 금방 다시 나오지 않게 합니다.
   - **차단 목록**에 걸리면 건너뜁니다.
   - **10분(600초) 초과 영상 제외** — 보통 믹스·루프·긴 라이브라 "노래가 아닌" 경우가 많아 거릅니다.
   - 통과하는 곡이 나올 때까지 **최대 8회 재시도**하고, 매 시도를 로그로 남깁니다.
4. **미리 받아두기(preview)** — 현재 곡이 재생되는 동안 **다음 추천 후보를 미리 골라 캐시에 받아 둡니다.** 그래서 곡이 끝나면 **끊김 없이 즉시** 다음 곡이 시작됩니다.

자동추천을 끄면 대기열에 남은 곡만 재생하고, 큐가 비면 멈춥니다. (반복 모드가 켜져 있으면 자동추천 대신 반복이 우선합니다.)
...
**소스 공개(source-available)이지만 오픈소스는 아닙니다.** — [LICENSE](LICENSE) 참고.

- ✅ 허용: 소스 열람·학습, **본인이 직접 빌드해 개인·비상업 용도로 실행**, 본인용 수정.
- ❌ 금지: 재배포·재공개·판매·서브라이선스(소스/바이너리 모두), **상업적 이용**(유료든 무료든 남에게 서비스로 제공 포함).

명시되지 않은 모든 권리는 저작권자에게 유보됩니다. 다른 용도가 필요하면 별도 허락을 받으세요.
