# 계획 05 — 웹에 우리가 직접 소리를 보내기

작성: 2026-09-10 · 상태: **구현 · 로컬 검증 완료. 후속 승인으로 main 병합·푸시 및 예약 배포 진행.** · **선행 조건이 있다 — §0 을 먼저 읽을 것**

> "진짜 송출을 해버려? 디스코드랑 같은 프로토콜로 보내면 되는거라"
> "근데 문제는 내컴에서 모든유저한테 음성파일이나 신호를 보내는건 유저가 많아질수록 힘들다는거야"

목표는 안드로이드에서 숨은 임베드가 즉시 멈추는 문제를 우리 `<audio>` 재생으로 줄이는 것이다.
대역폭은 실제 제약이며, 25초 에뮬레이터 결과를 장시간·실기기·iOS 보장으로 확대하지 않는다.

---

## 0. 실측 — 끝났다 (2026-09-13, 안드로이드)

> **이 절은 원래 '재기 전에는 한 줄도 쓰지 마라' 였다.** 그리고 표에 *"안드로이드가 이미 잘
> 된다 → v4.57 로 충분했다"* 라고 적어 뒀는데 **그게 틀렸다.** 문제는 아이폰이 아니라
> **안드로이드**였고, v4.69 를 배포한 뒤에도 안 고쳐졌다. 아래는 그걸 실제로 잰 결과다.

재는 장비를 만들어 뒀다 — `scripts/Test-AndroidBackgroundAudio.ps1`. 에뮬레이터(API 36,
`google_apis_playstore`)에 실제 크롬을 띄워 두 갈래를 같은 조건으로 돌린다. 남의 폰을
빌리지 않고 고칠 때마다 다시 잴 수 있다.

### 결과

| 갈래 | JS 판정 | 플랫폼 신호 (`dumpsys audio` 의 `state:started`) |
|---|---|---|
| **A** 우리 오리진 `<audio>` (128k opus) | **계속 재생됨** · 벽시계 24.42s / 진행 24.42s (비율 1.000) | 전경 1 → 배경 **1, 1, 1** |
| **B** 유튜브 1×1 iframe (지금 방식) | **멈췄음** · `getPlayerState()` 1→**2**(PAUSED), 시간 동결 | 전경 1 → 배경 **0, 0, 0** |

숨는 순간(`document.hidden`) B 는 곧바로 멈춘다. A 는 25초 내내 1초당 1.00초씩 흐르고
`paused:false` 였다 — JS 조차 안 졸았다(소리가 나는 페이지는 크롬이 덜 조인다).

**대조군이 결론을 지탱한다.** B 가 같은 브라우저·같은 세션에서 멈추는 것을 같이 보지 못하면,
A 가 살아남은 것이 "에뮬레이터가 원래 배경 정책을 안 걸기 때문" 인지 구별할 수 없다.
둘이 갈렸으므로 에뮬레이터는 정책을 걸고 있고, A 의 성공은 진짜다.

### 이 측정이 증명하지 못하는 것

정직하게 적는다. 셋 다 결론을 바꿀 수 있다.

1. **정지 주체를 단정할 수는 없다.** Chrome 61 문서는 MSE 최적화의 제한된 설명이다.
   현재 모든 교차 출처 임베드 정책을 배제하는 증거가 아니다. A/B 실측은 재생 경로를
   바꿀 근거지만 유튜브 내부 코드의 원인까지 증명하지 않는다.
2. **"크롬이 배경 웹오디오를 죽인다" 는 배제됐다.** 이건 확정이다 — A 가 같은 환경에서 살았다.
3. **제조사 절전은 아직 안 쟀다.** 에뮬레이터는 맨 안드로이드다. 삼성 One UI·샤오미는
   프로세스를 더 적극적으로 죽인다. 여기서 통과해도 실기기 확인이 남는다 —
   30분 이상 곡 경계를 넘겨서, 화면 잠금, 절전 모드, 블루투스 끊김, 밤새 한 번.
   **25초 홈 시험은 "즉시 멈추는가" 만 답한다.**

---
## 1. 대역폭 — 평균 소비와 공유 회선의 한계

실측 (봇 호스트 캐시 3,614곡): **평균 3.77 MB**, 최소 0.24 MB, **최대 286.74 MB**, 총 13.32 GB.

```
128 kbps = 16 KB/s = 57.6 MB/시간   (1명당)
```

| 동시 청취자 | 업로드 | 하루 4시간·30일 |
|---|---|---|
| 10명 | 1.28 Mbps | 69 GB |
| 20명 | 2.56 Mbps | 138 GB |
| 30명 | 3.84 Mbps | 207 GB |

오디오 128k 는 1080p 영상(약 5Mbps)의 **1/39** 다. "유저 많아지면 힘들다" 는 영상
스트리밍보다 요구량이 작다. 그래도 청취자 수에 비례하는 업로드와 데이터 비용은 남는다.

**참고 — 지금 웹 청취자는 실측 최대 1명이다.** 19일치 로그에서 웹 관련 줄이 9개뿐이다.
이 기능이 거의 안 쓰이고 있다는 뜻이고, 그 이유가 지금 웹 재생이 불편해서일 수도 있다
(백그라운드 안 됨, 곡 사이 무음, 임베드 금지 곡 재생 불가).

### 진짜 위험은 평균이 아니라 몰림이다

**모두가 같은 곡을 같은 지점에서 듣는다.** 그래서 곡이 바뀌는 순간이 **동시에** 온다.
10명이면 그 순간 10 × 3.77MB = **37.7 MB 가 한꺼번에** 나간다.

이건 **미리 받기로 풀린다.** 다음 곡을 앞 곡이 나오는 동안(3~4분) 나눠 받으면
37.7MB / 210초 = 180 KB/s 로 평탄해진다. 즉 §3 의 미리 받기는 편의 기능이 아니라
**몰림 방지 장치**다.

---

## 2. 하지 않기로 한 것 (그리고 그 이유)

### 조각내서 이어 붙이기 — 못 한다, 그리고 할 필요도 없다

"5분 이상이면 3분 단위로 잘라 받자" 는 발상은 문제를 정확히 짚었지만, 두 가지가 걸린다.

**하나. 우리 캐시 형식으로는 안 된다.** 브라우저에서 실측했다.

```
MediaSource.isTypeSupported          ← 조각을 이어 붙일 때 쓰는 것
  audio/ogg;  codecs=opus   →  false    ← 우리 캐시가 이것 (.opus = Ogg)
  audio/webm; codecs=opus   →  true
  audio/mp4;  codecs=mp4a   →  true

<audio> canPlayType                  ← 통째로 재생
  audio/ogg;  codecs=opus   →  probably ← 이건 된다
```

측정한 브라우저는 Ogg MSE를 지원하지 않는다. WebM으로 remux하는 선택지도 있지만
기존 캐시 전량을 반드시 변환해야 하는 것은 아니다. 이번에는 별도 컨테이너 경로를 만들지 않는다.

`<audio src>`의 점진적 Range 요청은 전체 파일 미리받기를 보장하지 않는다.
클라이언트는 작은 파일을 fetch/Blob으로 전부 받고, 32 MiB를 넘으면 네이티브 audio의 Range로 듣는다.
길이가 아니라 **실제 바이트 크기**가 메모리 예산을 결정한다. 큰 파일의 Range 서버 기능과
포털의 전체 미리받기는 별개의 경로다.

### 캐시 키와 응답 캐시 정책

`TrackRef::cache_key()` 는 제공자와 ID를 합친 **논리 키**다. 재인코딩·SponsorBlock·
원본 편집으로 바이트가 달라지므로 내용 주소가 아니다. 이번 구현은 큐 항목 ID URL과
실제 파일 SHA-256 ETag, `Cache-Control: private, no-cache`를 쓴다. 매 요청에서 인가를
다시 확인하고 공용 CDN 캐시는 금지한다. 이미 받은 Blob의 강제 회수는 불가능하다. 내려받은 Blob은 해당 탭에서만 재사용한다.
장기 `immutable`은 실제 내용 주소와 접근 철회 정책을 함께 설계하기 전까지 쓰지 않는다.

### 싱크 포기 — 아낄 것이 없다

`WEB-PLAYER-DESIGN.md:6` 의 최상위 요구가 *"사람마다 반드시 같은 곳, 같은 노래"* 다.
그리고 **싱크는 이미 공짜로 있다** — `startedUtc` 절대시각 + 시계 오차 보정이 그대로
쓰인다. 오히려 `<audio>` 로 바꾸면 **쉬워진다**(§5). 포기할 이유가 없다.

---

## 3. 구현 구조

| 파일 | 역할 |
|---|---|
| `web/stream.rs` | 전역 전송 큐·본문 속도 제한, 단일 Range/HEAD/ETag, 취소·시한으로 슬롯 반납 |
| `web/remote.rs` | 항목 ID 라우트, 기존 로그인/길드 인가 + 현재/다음 곡 허용 목록, 상태 payload 네 경로와 watcher 서명 |
| `remote/models.rs` · `remote/store.rs` | 전역 `WebStreamSettings`, 기본 꺼짐·10전송·2,000 kbps, 저장/읽기 범위 제한 |
| `player/coordinator.rs` | 웹 전용 현재 곡을 시각표와 분리해 비동기 prepare, 중복 시도 억제 |
| `media/cache.rs` · `web/pages.rs` | 전송 파일 pin, LRU/전체/수동 삭제 보호 |
| `portal.js` · `core.js` | stream 상태 전달, audio 두 개·Blob 두 곡, 명시적 탭·직접 받기 토글, 싱크/볼륨/OS 일시정지, 폴백 |
| `console.js` | 봇 주인 전용 전역 전송 수·대역폭 설정 |
| `sw.js` | 변경 없음. 기존 `/music/api/*` 통과 |
| `Cargo.toml` | bounded channel을 body로 옮기는 futures-util, 테스트 router용 tower util |

`ReaderStream`만 붙이면 본문 poll이 멈춘 동안 시한도 멈춘다. 독립 생산 태스크에
절대 시한을 두고, 1청크 bounded channel로 메모리와 역압을 제한한다.
설정·응답 세부 계약은 `REMOTE-API-V4.md`의 웹 직접 받기 절이 기준이다.

---

## 4. 전송 큐와 전역 대역폭 예산

### 청취자가 아니라 응답 본문 전송을 제한한다

봇 주인이 전역 기능을 켜고, 사용자가 `🔊 직접 받기`를 켠 경우만 파일을 받는다.
기본은 둘 다 꺼짐이다. 전역 동시 전송 기본 10개, 범위 1~30개다.
파일을 받는 동안만 슬롯을 점유하고 완료·취소·오류·시간 초과에 반납한다.
듣는 시간이나 WebSocket 90초 유예와는 별개다. 이미 받은 Blob으로 들을 때 슬롯은 0개다.

기본 시도 순서는 **원본 CDN → 봇 서버 캐시 → 임베드**다. 직접 받기 준비 중에는 진행 상태를
보여 주며 임베드를 먼저 켜지 않는다. 서버는 현재 곡 요청을 먼저, 다음 곡 요청은 곡 종료
마감 순으로 처리한다. 미리받기는 전체 슬롯을 차지하지 못하게 현재 곡용 여유를 남긴다.
큐와 사용자별 요청 수에도 상한을 두고, 만료·재정렬·스킵 뒤에는 인가를 다시 검사한다.
대기열 포화나 일시 실패는 재시도 가능한 응답으로 돌려주며 클라이언트는 지연 후 다시 요청한다.

### 슬롯 수를 회선 속도에 곱하면 안 된다

20 Mbps 회선에서 3.77 MB 하나는 약 1.51초다. 하지만 슬롯 10개가 모두 20 Mbps를
얻는 것이 아니다. 동시에 10개면 **합계** 20 Mbps를 나눈다. 235초 동안 이 회선의
이론상 총 전달량은 슬롯 수와 무관하게 약 156곡분이다. 지시서의 1,560회는 10배 과대다.
100명은 평균 약 12.8 Mbps, 곡마다 377 MB가 필요하다. 따라서 100명 이상 수용은
실측 업링크와 설정한 웹 예산이 충분할 때만 가능하고 슬롯 10개 자체가 보장하지 않는다.

### 최종 방어는 호스트 전체 바이트 예산이다

전역 설정 `bandwidthKbps` 기본 2,000, 범위 128~20,000 kbps(십진 bit/s).
0은 무제한이 아니라 잘못된 값이다. 기존 큐 길이의 0=무제한 규칙과 다른 **안전 상한**이다.
모든 길드·사용자·Range·재시도가 한 속도 제한기를 공유한다.
작은 청크를 시간에 따라 내보내며, 느린 연결에도 응답 수명 상한을 둔다.
사용자별 분당 바이트 제한도 걸어 한 사람이 재요청으로 전체 예산을 독점하지 못하게 한다.
기본 2 Mbps는 약 15명분의 평균 오디오이고, 100명 성능을 약속하는 값이 아니다.

어떤 가정용 회선에서도 안전하다는 숫자는 없다. 설정 상한도 실제 업링크를 모르므로
안전 보증이 아니다. 운영자는 실측 업로드에서 Discord와 다른 트래픽·여유를 빼고 정한다.
이 제한은 앱 본문 바이트 기준이며 TCP/TLS 헤더와 외부 프록시 버퍼까지 정밀 제어하지는 않는다.

### 미리받기에는 크기 상한이 필요하다

브라우저 `preload=auto`는 전체 다운로드 보장이 아니다. **32 MiB 이하** 파일은
명시적 fetch → Blob으로 전부 받은 뒤 DOM에 유지한 두 audio를 교대로 쓴다.
현재/다음 두 곡만 유지하고 바뀐 후보 요청은 취소, Blob URL은 해제한다.
32 MiB 초과는 동일 오리진 URL을 `<audio src>`에 연결해 필요한 구간을 Range로 받는다.
MSE나 Opus 조각 이어 붙이기가 아니므로 remux가 필요 없다. 전체 파일을 Blob으로 만들지 않는다.
큰 요청도 최대 1 MiB의 206으로 응답해 슬롯·대역폭 예산을 계속 공유한다.
재생 중 다시 슬롯과 네트워크가 필요하므로 전체 선다운로드·오프라인 재생을 보장하지 않는다.
라이브·미지원 코덱·재생 실패에는 임베드 폴백을 유지한다. 크기만으로 폴백하지 않는다.

다운로드 패널은 **서버 캐시 준비 상태**와 **이 탭의 현재·다음 곡 보관 상태**를 구분한다.
Blob은 받은/남은 바이트·진행률·전송 평균 기준 예상 시간을 표시하고 큐 대기는 추정하지 않는다.
Range는 브라우저가 받은 바이트를 정확히 공개하지 않으므로 현재 위치 이후 연속 버퍼 초를 표시한다.
사용자가 제공한 운영 실측은 6,067곡 중 32 MB 초과 6곡(0.10%, 평균 3.84 MB)이다.
이는 독립 재측정이 아니며, 드물다는 이유로 해당 곡의 직접 재생을 포기하는 근거로 쓰지 않는다.

캐시 미스에서는 웹 전용 현재 곡도 비동기 prepare한다. **웹 전용 첫 곡에는 새로운
다운로드/변환 부하가 있다.** 항상 공짜라는 초안은 틀렸다. 전송 중 파일은 RAII pin으로
LRU·전체 비우기·수동 삭제·마이그레이션에서 보호한다. 이미 Blob을 받은 청취자는 pin이 불필요하다.

---

## 5. 덤으로 풀리는 것

이 전환은 안드로이드의 즉시 배경 정지 때문에 하는 것이지만, 곁다리로 셋이 같이 풀린다.

**하나. 곡 사이 무음.** 지금 코드가 스스로 적어 뒀다 —
> `portal.js:6589` — "유튜브 플레이어가 **하나뿐**이라, 거기에 다음 곡을 cue 하는 순간 지금 곡이 내려간다. 진짜로 하려면 숨은 플레이어를 하나 더 두고 곡이 바뀔 때 둘을 바꿔 끼워야 한다(이중 버퍼). 그건 별도 작업이라..."

`<audio>` 는 둘 만드는 게 `new Audio()` 두 번이다.

**둘. 싱크가 쉬워진다.** 지금 `seekTo(t, true)` 는 **네트워크 재요청**을 유발해서
되먹임을 만든다(v4.55 가 쿨다운 6초로 땜질했다). 로컬 `<audio>` 는 `currentTime = t` 가
버퍼 안에서 즉시 끝난다. 게다가 `playbackRate = 1.002` 로 **seek 없이** 서서히 맞출 수
있다(0.2% = 3.5센트, 귀에 안 들린다). 지금 구조로는 불가능한 방법이다.

**셋. 임베드 금지 곡.** 실측으로 확인했다 — `ERROR 150`(업로더가 임베드를 막음)이 나면
지금은 웹에서 못 듣는다(`portal.js:6658-6659`). 우리가 보내면 들린다.
**다만 이건 §6 의 문제이기도 하다.**

---

## 6. 감수해야 하는 것

### 클라우드플레어

실측으로 확인했다 — 리모컨은 클라우드플레어 엣지를 지난다.

```
server: cloudflare · cf-ray: ...-HKG · cf-cache-status: DYNAMIC
```

터널과 CDN 이 별개 제품인 것은 맞지만 **바이트는 그들 망을 지나간다.** CDN 약관에
*"유료 서비스 없이 video, audio files, 또는 큰 파일을 서빙하면 제한할 권리"* 조항이
있고, 월 수백 GB 의 오디오는 문언상 해당한다. 실제 제재 여부는 규모·재량이라 예측 불가다.

피하려면 오디오만 별도 서브도메인을 회색 구름(프록시 끔)으로 두고 직결해야 하는데,
그러면 **집 IP 가 노출되고** 포트를 직접 열어야 한다 — `README.md:159` 의 권장과 정반대다.
**봇 주인이 이 위험을 알고 구현을 승인했다.** 전송 슬롯 10개는 월 사용량 제한이 아니며,
총 사용량은 실제 청취 시간과 바이트를 기준으로 판단해야 한다.

### 유튜브 약관 — **여기가 가장 무거운 항목이다**

2026-09-13 조사(서브에이전트 + Codex)로 확인했다. 사실만 적는다.

**① 지금 쓰는 1×1 임베드가 이미 규정 위반이다.** 이건 계획과 무관하게 지금 그렇다.

> *"Embedded players must have a viewport that is at least 200px by 200px"*
> — [required-minimum-functionality](https://developers.google.com/youtube/terms/required-minimum-functionality)

**② 배경 재생 자체가 명시적으로 금지돼 있다.** 즉 우리가 "고치려는" 동작은 버그가 아니라
의도된 차단이다.

> *"create, include, or promote features that play content, including audio or video
> components, from a background player, meaning a player that is not displayed in the page,
> tab, or screen that the user is viewing"* — 개발자 정책 §III.F.3
> — [developer-policies](https://developers.google.com/youtube/terms/developer-policies)

**③ 오디오만 떼어내는 것도 금지 조항에 그대로 있다.** 이 계획의 핵심이 바로 그것이다.

> *"separate, isolate, or modify the audio or video components of any YouTube audiovisual
> content"* — 같은 문서 §III.I

**④ 유튜브가 2026년 2월에 실제로 조이기 시작했다.** 구글이 공식으로 확인한 발언이다.
우회로가 점점 막히는 방향이라는 뜻이다.

> *"Background playback is a feature intended to be exclusive for YouTube Premium members. …
> we have updated the experience to ensure consistency across all our platforms."*
> — [9to5Google (2026-02-02)](https://9to5google.com/2026/02/02/youtube-background-playback-workarounds-not-working-third-party-browsers/)

**⑤ 이 사유로 앱이 플레이 스토어에서 실제로 내려갔다.** 개발자가 배경 재생 API 를 부르지도
않았고 유튜브 임베드 기본 컨트롤만 있었는데도 심사에서 걸렸다.

- [react-native-youtube-iframe #72](https://github.com/LonelyCpp/react-native-youtube-iframe/issues/72)
  — *"your app violates the Device and Network Abuse policy by enabling background play of YouTube videos"*
- 안드로이드용 유튜브 플레이어 라이브러리도 같은 경고를 문서에 박아 뒀다:
  *"you won't be able to publish your app on the Play Store"*
  — [android-youtube-player wiki](https://github.com/PierfrancescoSoffritti/android-youtube-player/wiki/Useful-info)

### 그래서 이 계획이 바꾸는 것은 무엇인가

**"약관을 안 지키던 것을 지키게 되는" 변화가 아니다.** 위반의 성격이 옮겨 간다.

| | 지금 | 이 계획 뒤 |
|---|---|---|
| 위반 조항 | §III.F.3 (배경 플레이어) · 200×200 | §III.I (오디오 분리) · 다운로드·캐싱 |
| 누가 재배포하나 | 각 브라우저가 유튜브에서 직접 받는다 | **우리 서버가 파일을 내려준다** |
| 노출 | 숨은 임베드 하나 | 서버가 오디오를 서빙하는 엔드포인트 |

디스코드 재생도 같은 다운로드를 하지만, 그건 **청취자 수와 무관한 한 스트림이고 파일이
사용자에게 넘어가지 않는다.** 이 계획은 그 선을 넘는다 — 파일이 사용자 브라우저로 간다.
그게 실질적인 차이다.

**봇 주인이 위험을 알고 구현을 승인했다.** 최초 지시는 배포 제외였고, 후속 지시로 배포·푸시·main 병합을 승인했다. 운영 중 재생을 끊지 않는 `-Stage` 예약 배포를 기본으로 한다.
### 캐시 미스 지연

웹 전용 모드의 첫 곡은 캐시에 없다. v4.56 이 기록한 "통째로 받아 재인코딩" 지연이
그대로 온다. `reconcile_virtual` 에 현재 곡 `prepare` 를 넣는 것이 **필수**다(§3).

### 캐시 정리가 재생 중인 파일을 지울 수 있다

`cache.rs` 의 LRU 정리는 지금 "재생 중"을 모른다. 스트리밍 중인 파일을 지우면 그 사람의
소리가 끊긴다. 슬롯이 잡고 있는 `cache_key` 는 정리 대상에서 빼야 한다.

---

## 7. 왜 다른 길이 없는가 (2026-09-13 조사)

기존 임베드를 부모 페이지에서 제어하는 대안의 제약을 조사했다. **모든 가능한 구현의
불가능성을 증명한 것은 아니다.** 이번에는 실측으로 개선이 확인된 audio 경로를 선택한다.

### 재생 경로 변경의 근거와 한계

[Chrome 61 문서](https://developer.chrome.com/blog/media-updates-in-chrome-61)는 MSE 비디오
최적화에 관한 과거 문서다. 현재 모든 배경 정지를 유튜브만의 동작이라고 단정할 수 없다.
교차 출처 iframe 내부를 부모에서 수정할 수 없고 A/B 실측에서 audio가 살아남았으므로,
우리가 제어하는 audio 경로가 이번 구현의 근거다. 아래 대안 비교는 일반적인 제약이며
모든 OS·앱에 대한 불가능성 증명이나 장시간 재생 보증으로 읽지 않는다.

### 시도해 볼 만해 보였던 것들 — 전부 아니오

| 방법 | 되나 | 왜 |
|---|---|---|
| `allow="autoplay"` | ✘ | 자동재생 권한만 위임한다. 정지와 무관 |
| 부모에서 PiP 호출 | ✘ | `requestPictureInPicture()` 는 **내 문서의** `<video>` 에만 부를 수 있다. IFrame API 에 PiP 메서드도 없다 |
| Document PiP (iframe 을 PiP 창에) | ✘ | **안드로이드 크롬 미지원** |
| MediaSession 핸들러로 다시 `playVideo()` | ✘ | 다시 멈춘다. 반복해서 싸우는 건 금지 조항 우회 시도다 |
| Web Worker · 서비스워커 | ✘ | 미디어 요소를 소유하지 않는다. 페이지 가시성을 바꿀 수 없다 |
| Wake Lock · 무음 오디오 · 소켓 심박 | ✘ | 배경 자격을 주지 않는다 |
| PWA 로 설치 | ✘ | 설치는 임베드의 재생 계약을 바꾸지 않는다 |

PiP 는 명세가 못 박아 준다 — 알고리즘이 `HTMLVideoElement` 에 정의돼 있고 호출 문서의
transient activation 을 요구한다. 교차 출처 프레임에는 둘 다 없다.
[W3C Picture-in-Picture](https://www.w3.org/TR/picture-in-picture/)

### APK 는 답이 아니다 — 이 일의 **하류**다

| | 결과 |
|---|---|
| **TWA** | WebView 가 아니라 **크롬**을 쓴다(Custom Tabs). 유튜브 정지를 그대로 물려받는다 |
| **WebView 껍데기** (Capacitor·Cordova) | **크롬보다 나쁘다.** WebView 는 창 가시성이 바뀌면 미디어를 멈춘다 — 크롬에 없는 WebView 고유 동작이다. `onWindowVisibilityChanged` 를 네이티브로 덮어 억지로 살릴 수는 있는데, 그러면 **알림·잠금화면 컨트롤이 사라지고** 스토어에서 내려간다 |
| **네이티브 플레이어** (Media3/ExoPlayer + `MediaSessionService` 포그라운드 서비스) | 확실히 튼튼하다. **그런데 재생할 스트림이 먼저 있어야 한다** |

마지막 줄이 핵심이다. 네이티브 앱도 유튜브를 직접 못 튼다 — 우리 서버가 파일을 내려줘야
한다. 즉 **1단계(`stream.rs`)를 건너뛰는 길이 아니라, 그 위에 얹는 추가 공사다.**
[Android 배경 재생](https://developer.android.com/media/media3/session/background-playback)

### 우리 오리진 `<audio>` 는 실측으로 확인한 개선 경로다

크롬 안드로이드는 미디어 재생에 알림·잠금화면 컨트롤을 제공한다.
알림이나 DOM에 audio를 유지하는 것만으로 프로세스 수명과 장시간 재생이 보장되지는 않는다.
[미디어 알림](https://developer.chrome.com/blog/media-notifications)

조건이 몇 개 붙는데, 구현할 때 그대로 지켜야 한다.

- **5초보다 긴 미디어만** 알림이 뜬다.
- **Web Audio 만으로 소리를 내면 알림이 없다.** 반드시 `<audio>` 요소를 거쳐야 한다.
- `<audio>` 요소를 **항상 DOM 에 살려 둔다.** 붙였다 떼면 재생 권한이 흐트러진다.
- `metadata`와 `play`/`pause`를 채운다. 서버 곡을 주머니에서 바꾸지 않도록 이전/다음 곡은 비활성화한다.

### 남은 진짜 위험은 제조사 절전이다

API 문제가 아니라 **기기 설정 문제**다. 샤오미·삼성·화웨이는 맨 안드로이드 위에 자체
절전을 얹어 배경 앱을 얼린다(삼성은 기본으로 사흘 안 쓴 앱을 잠재운다).
[dontkillmyapp.com](https://dontkillmyapp.com/problem)

사용자 쪽 해결은 한 줄이다 — **설정 → 앱 → 크롬(또는 설치한 PWA) → 배터리 사용량 →
"제한 없음".** 이걸 안내 문구로 넣어야 한다.

오래 배경에 있으면 결국 멈추는 크로미움 버그도 열려 있다 —
[issue 375973479](https://issues.chromium.org/issues/375973479)

---

## 8. 구현할 때 틀리기 쉬운 것 (Codex 검토)

계획 초안에 있던 낙관을 고친다.

- **`<audio>` 두 개로 진짜 무간격은 안 된다.** `ended`·`timeupdate`·타이머는 샘플 단위가
  아니고, Opus 의 pre-skip/end padding 과 디코더 기동 시간이 있다. B 를 미리 완전히 준비해
  A 가 끝나기 조금 전에 시작하고 크로스페이드하면 **주관적으로** 촘촘해진다. 코덱 수준
  무간격은 약속하지 마라.
- **Range 만으로 탐색 정확도가 보장되지 않는다.** 컨테이너 타임스탬프·pre-skip·연속성을
  실제 디코더에서 확인해야 한다. 바이트 범위는 바이트만 준다. 이번 HTTP 테스트의 임의 바이트와
  브라우저 Opus 디코딩 테스트는 구분한다. SponsorBlock으로 길이가 달라진 기존 캐시는 별도 위험이다.
- **`immutable` 보다 내용 주소가 먼저다.** 순서가 바뀌면 잘못 만든 파일이 캐시에 박혀
  안 빠진다. `cache_key` 는 내용 주소가 아니다. 이번에는 인가 재검증이 있는 private/no-cache를 쓴다.
- **`206`·`Content-Range`·`Accept-Ranges`·MIME·검증자(ETag)를 준다.** 동일 오리진이라 CORS 허용은 추가하지 않는다.
- **MediaSession 은 문서 단위다.** 두 `<audio>` 사이에 "인계" 같은 것은 없다. 핸들러 한 벌을
  두고 어느 쪽이 주인이 됐을 때 `metadata`·`positionState` 만 갱신한다.
- **두 요소가 동시에 소리를 내지 않게 한다**(의도한 크로스페이드 구간 제외). 안 그러면
  크롬·시스템의 미디어 상태 판정이 흐려진다.
- **첫 재생은 탭이 직접 `play()` 를 불러야 한다.** 사이에 `await` 를 끼우면 사용자 활성화가
  소모돼 거절된다. 불러오기·디코딩·MediaSession 설정은 활성화를 만족시키지 않는다.
- **브라우저 캐시는 할당량 관리 대상이라 언제든 비워진다.** 미리받기는 최적화일 뿐
  저장소로 취급하면 안 된다.

---
## 단계와 검증 기록

1. 계획 검토/수정: 회선 계산·인원 상한·캐시 정책·메모리·근거의 범위를 고쳤다.
2. 서버 전송/전역 설정/현재 곡 prepare/캐시 보호 후 workspace 테스트 통과(347 + 11 + 1).
3. 포털 전환/이중 버퍼/관리 콘솔과 HTTP 통합 테스트를 추가했다.
4. Chromium에서 실제 포털과 Opus 디코더로 임베드 대기 → 직접 재생 → OS 일시정지/재개 →
   다음 곡 Blob 재사용 → 폴백 → 자동재생 거절/사용자 탭 → 종료를 검증했다.
   외부 임베드와 파일 응답·곡 일정은 fixture다. 실서버 송출을 실측한 결과는 아니다.
5. API 계약·v3의 부담 0 문장·v4.72 패치노트와 에셋 해시를 갱신한다.
6. 마지막 workspace 테스트 결과와 제한 사항을 작업 완료 보고에 남긴다.

안드로이드의 원래 25초 A/B 결과는 §0에 있다. 새 전체 포털의 실기기 30분 이상
곡 경계·잠금·절전·블루투스 검증 및 iOS는 남아 있다. 예약 빌드는 다음 봇 재시작 때 적용하며, 직접 송출의 전역 기본값은 꺼짐으로 유지한다.

---

## 하지 않기로 한 것

| | 왜 |
|---|---|
| 3분 단위 직접 조각내기 | MSE 가 Ogg 를 안 받는다(실측). 그리고 브라우저가 이미 한다 |
| 5분 기준 길이 분기 | 32 MiB 이하 Blob, 초과는 native Range. 시간 대신 실제 바이트로 메모리를 제한 |
| 장기 immutable 캐시 | 논리 키는 내용 주소가 아니며, 인증된 파일의 공용 캐시는 금지 |
| 64k 로 낮춰 보내기 | 절대량이 이미 작아 이득이 작고, 곡마다 재인코딩 + 캐시 2배가 든다. 모바일 데이터 절약 옵션으로는 뜻이 있다 |
| 백그라운드일 때만 전환 | iOS 는 백그라운드에서 JS 가 멈춰 전환 코드가 안 돈다. 안드로이드도 전환 순간 끊긴다 |
| 전면 교체 (iframe 제거) | 폴백이 없어진다. 임베드는 남겨 두는 것이 상한·실패의 안전망이다 |
| 싱크 포기 | 최상위 요구와 충돌하고, 아낄 것도 없다 |


## 9. 이번 검토에서 바꾼 이유 (2026-09-13)

### 후속 요구: 원본 우선·설정 단일화

- 서버는 익명 yt-dlp로 현재/다음 곡의 공개 HTTPS 음원 주소만 조회한다. 음원 바이트는
  클라이언트가 CDN에서 받는다. 호스트 쿠키·프로필·추출 결과의 HTTP 헤더는 내보내지 않는다.
- 작은 곡은 CORS fetch/Blob, 큰 곡·크기 미상은 native audio. fetch가 막히면 같은 원본의
  native audio도 시도하고, 실패하면 서버 캐시, 마지막에 임베드로 넘어간다.
- 짧은 현재·다음 곡은 전부 받고, 긴 현재·다음 곡은 `preload=auto`로 준비한다. 이것은
  브라우저 힌트이므로 특정 3분 버퍼나 네트워크 단절 이후 재생을 보장하지 않는다.
- 원본 URL은 IP·헤더·만료 제약이 있을 수 있다. 실제 AVD의 공개 영상 1건은 주소 추출은
  성공했지만 fetch와 native audio가 모두 실패했다. 원본 직접 받기의 보편적 성공 증거가 아니다.
- `/music/owner`가 단일 봇 주인 재생 설정 화면이다. 경로 순서/토글·미리받기·전역 기본값·
  강제값을 모으며, 운영 사이트에는 OAuth·쿠키·호스트 도구만 남긴다. 두 인증을 합치지 않는다.
- 기본값은 origin → server → embed, 모두 허용·다음 곡 미리받기 켜짐이다. 직접 받기 기능의
  전역 기본 꺼짐은 유지한다. 경로를 모두 끄면 직접 받기에서 무음과 안내를 표시한다.

### 후속 AVD 검증 (배포 전)

- Android 16 AVD의 Chrome에서 실제 포털·Opus 디코더를 사용했다. 곡 일정과 음원 서버는
  로컬 fixture이며 원본 CDN 성공 경로는 별도 브라우저 fixture 테스트다.
- 작은 파일 Blob과 38.5 MB / 35분 파일 Range 각각 홈 45초·잠금 45초·잠금 중 곡 경계
  110초를 검사했다. 측정 구간에는 CDP를 끊었고, Android 오디오 재생 상태 표본은 모두 활성이다.
- 두 경로 모두 숨김 상태에서 다음 곡의 `playing` 이벤트를 확인했다. Blob 곡 경계의
  `ended`→`playing` 이벤트 간격은 약 68 ms였다. 샘플 단위 무간격의 증거는 아니다.
- 긴 파일 검사는 중간 탐색과 끝부분의 경계 검사다. 35분 전체 연속 재생을 측정한 것은 아니다.
  제조사 실기기 절전·30분 이상 잠금·블루투스·iOS 검증은 여전히 남아 있다.
- 재현: `scripts/Test-AndroidWebDirectStream.cjs`, Range 모드는 `PLAN05_RANGE=1`.
  위 결과는 v4.73 후속 변경의 배포 전 검증 기록이다. 배포 적용 여부는 운영 빌드 ID로 확인한다.

### 최초 검토 기록

- §4를 청취 인원 상한에서 전송 슬롯·우선순위 큐·호스트 바이트 예산으로 교체했다.
- 20 Mbps를 슬롯 수만큼 곱하던 계산, 캐시 키=내용 주소, 웹 전용 변환 비용 0을 정정했다.
- 인증 쿠키가 있는 파일에 public/immutable을 주면 접근 철회가 우회되므로 private/no-cache로 바꿨다.
- 전체 미리받기는 브라우저의 preload 힌트와 다르다. Blob 크기와 수명 상한을 명시했다.
- 후속 검토: "배경 재생에는 전체 파일이 반드시 있어야 한다"는 과장이다. 네이티브 Range도
  가능하며, 대신 버퍼를 소진하면 네트워크가 필요하다. Ogg MSE 미지원과 native audio 지원은 별개다.
- 206에 요청 범위 일부만 보내는 것은 [RFC 9110 §15.3.7](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.3.7)이
  허용한다. 최대 1 MiB와 실제 Content-Range를 반환하고 나머지는 브라우저가 요청한다.
- 첫 탭에서 준비된 audio.play()를 직접 호출한다. 아직 파일이 없으면 전환 시 NotAllowedError를
  받아 재생 버튼을 안내하며, 무음 재생으로 권한을 얻었다고 가정하지 않는다.
- 예정된 다음 곡은 확정이 아니다. 두 번째 audio는 준비만 하고 서버의 실제 곡 전환 뒤에 튼다.
  샘플 단위 무간격이나 JS가 동결된 상태의 자동 곡 전환은 보장하지 않는다.
- HTTP 의미 참고: [Cache-Control](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Cache-Control),
  [Range](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Range).
