...
### 진짜로 무서운 것 — **업링크를 디스코드와 나눠 쓴다**

대역폭 계산에서 빠뜨렸던 것이 있다. 웹으로 내보내는 바이트와 **디스코드 음성으로 나가는
바이트가 같은 회선을 쓴다.** 업로드가 포화되면 먼저 망가지는 것은 새 기능이 아니라
**지금 잘 돌던 디스코드 재생**이다. 듣던 사람들의 소리가 끊긴다.

그래서 상한은 "서버가 버티는 수" 가 아니라 **"디스코드를 해치지 않는 수"** 로 잡아야 한다.

- 기본 10명(1.28 Mbps)은 어떤 가정용 회선에서도 안전하다. 1080p 영상 한 명(약 5 Mbps)의
  **1/4** 이다.
- **설정에 하한·상한을 박는다.** 이 저장소는 이미 그렇게 한다(`autoplay_seed_max.min(100)`
  같은 식). 누가 500 을 넣어 회선을 포화시킬 수 있게 두면 안 된다. 상한 30 을 제안한다 —
  30명이면 3.84 Mbps 로, 그 이상은 가정용 업로드에서 디스코드와 다투기 시작한다.
- 올리기 전에 **실제 업로드 속도를 재고** 정한다. 문서에 숫자를 박지 않는 이유는 회선마다
  다르기 때문이다.

### 반대로, 걱정 안 해도 되는 것

**CPU 와 디스크는 거의 공짜다.** 이유를 분명히 해 둔다.

- 캐시가 **이미 128k opus** 다(`cache.rs:344` 의 `libopus -b:a 128k`). 웹으로 보낼 때
  **재인코딩이 없다.** 파일을 그대로 흘려보낸다.
- 그 파일은 **디스코드 재생 때문에 어차피 받는다.** 이 기능이 새로 만드는 변환 부하는
  **0** 이다. 늘어나는 것은 나가는 바이트뿐이다.
- 10명이 같은 곡을 들으면 디스크는 **한 번만** 읽는다. 나머지는 OS 페이지 캐시(RAM)에서
  나간다. 모두가 같은 지점을 듣는 이 설계에서는 오히려 유리하다.

### 한 자리가 폭주하는 경우

상한은 **동시 인원**을 막지 바이트를 막지 않는다. 한 사람이 탐색을 반복하거나 클라이언트가
잘못 만들어져 같은 구간을 계속 다시 받으면, 슬롯 하나가 열 명 몫을 쓸 수 있다.

1차 방어는 **내용 주소 URL + `immutable`** 이다(§8). 제대로 걸면 브라우저가 다시 안 받는다.
그래도 서버가 스스로를 지켜야 하므로, 슬롯마다 **분당 바이트 예산**을 두고 넘으면 그 세션만
임베드로 되돌린다. 전체를 막지 않는다 — 이 설계의 실패 모드는 언제나 "예전처럼 듣는다" 다.
### 슬롯을 언제 놓는가

이게 안 되면 유령 슬롯이 쌓여 아무도 못 켠다. 이미 있는 장치를 그대로 쓴다 —
`WEB_LISTENER_GRACE`(90초, `remote.rs:5355`)와 같은 스위퍼에 얹는다. 소켓이 끊기고
