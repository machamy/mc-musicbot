//! mc-musicbot 본체 — Rust(serenity + songbird) 디스코드 음악봇.
//! 오디오 엔진: songbird (전용 스레드 페이싱 + DAVE E2EE 내장) — 끊김 방지의 구조적 해법.
//! 데이터: .musicbot-data/musicbot.sqlite (SQLite).

/// 자산 주입 통로. **`mc-assets` 를 직접 의존하지 않기 위한 것이다** — 이유는 모듈 문서에.
pub mod assets_di;
pub use assets_di::{Assets, install_assets};
...
pub async fn run() {
    ...
    match media::ytdlp::init_js_runtime(&app.config.yt_dlp_path).await {
        ...
        /* **없는 문제를 경고하고 있었다.**
         *
         * 예전 문구는 "JS 런타임을 못 찾았습니다. 유튜브 곡이 403 으로 실패할 수 있어요
         * — yt-dlp 옆에 deno 를 두면 해결됩니다." 였다. 세 군데가 틀렸다.
         *
         * 못 찾은 주체는 **우리**(`js_runtime_beside` 는 yt-dlp 옆 폴더 하나만 본다)이지
         * yt-dlp 가 아니다. 요즘 yt-dlp 는 PATH 에서 스스로 찾아 쓴다(실측: stderr 에
         * `[jsc:deno]` 가 찍힌다). 그리고 런타임이 아예 없어도 서명 없는 경로로 우회해
         * 곡이 받아진다 — 403 이 난다는 예측도 사실이 아니다.
         *
         * 그래서 이 줄은 운영자를 폴더 옮기기에 붙잡아 두고 **진짜 원인(낡은 yt-dlp)에서
         * 눈을 떼게** 만들었다. 단정하지 말고, 우리가 아는 만큼만 말한다. 실제로 무엇이
         * 쓰이는지는 곡을 한 곡 받아 보면 stderr 가 알려 주고, 그때 `observe_stderr` 가
         * 잡아 둔다. */
        None => app.log.info(
            "Tools",
            "JS 런타임 위치를 못 박지 못했습니다 — yt-dlp 가 PATH 에서 스스로 찾아 쓸 수 \
             있으니 곡이 잘 받아지면 그대로 두세요. 곡을 하나 튼 뒤 이 로그에 무엇이 \
             쓰이는지 다시 적힙니다.",
        ),
    }
