...
///
/// **하루보다 길게 둔다.** 외부 차트는 하루 단위로 움직여서 그보다 자주 긁어도 같은 목록이고,
/// 받는 데는 yt-dlp 가 곡마다 돌아 몇 초씩 걸린다. 짧게 두면 얻는 것 없이 사람이 기다리는
/// 시간만 늘어난다. 노래방(`KARAOKE_CACHE_TTL_HOURS`)이 먼저 이 결론에 도달했고 같은 이유다.
/// 지금 당장 최신이 필요하면 관리자가 `차트 다시 가져오기` 로 언제든 강제할 수 있다.
pub const CHART_CACHE_TTL_HOURS: i64 = 30;
...
/// 차트 곡 수 기본값.
///
/// **원본이 주는 만큼 다 보여준다.** TJ 는 `topAndHot100` 이라는 이름 그대로 100곡을 주고,
/// 유튜브 "Top 100" 재생목록들도 100곡이다. 50 으로 잘라 두면 **사이트에 있는 절반을
/// 버리는 것**이라 "왜 50개까지만 되냐" 는 말이 나온다.
///
/// 느려지지 않는 이유: TJ 는 곡마다 반주를 찾아야 하지만 한 번에 25곡씩만 새로 찾고
/// (`RESOLVE_PER_FETCH`) 나머지는 다음 조회와 유휴 프리페치가 이어서 채운다.
/// 유튜브 재생목록은 한 번에 받아 오므로 개수와 거의 무관하다.
