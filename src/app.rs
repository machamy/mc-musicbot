...
///
/// **재생 중일 때는 건너뛴다** — 프리페치가 yt-dlp 를 붙들면 정작 곡을 트는 쪽이 밀린다.
/// 한 번에 한 장만 하는 이유도 같다. (화면을 열어 둔 사람이 있는 것만으로는 미루지 않는다.
/// 리모컨을 켜 두는 게 정상 사용이라, 그걸로 미루면 프리페치가 영영 안 돈다.)
...
/// 프리페치 주기. 캐시 수명(`CHART_CACHE_TTL_HOURS`)보다 훨씬 촘촘해야
/// "식자마자 채워지는" 상태가 유지된다. 10분이면 하루에 최대 144장을 데울 수 있다.
